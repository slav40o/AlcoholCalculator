...
angular.module('app.services')
  .factory('drinksService', function ($http, $q, logService, messageService, localStorageService, calculationsService, uuidService) {
    'use strict';

    function addDrink(drink) {
      var drinks = getHistory(),
        result = addDrinkElement(drink, drinks);
      setHistory(drinks);
    ...
    function addDrinkElement(drink, drinksList) {
      ...
      if (bodyWater <= 0) {
        return messageService.constructMessage('Body water is not calculated!', 'Body water should be calculated when the account information is changed.');
      }
      ...
      var result = messageService.constructMessage('Successfully added!', 'Drink was added to the drink history');
    ...
    function getById(id) {
      ...
      if (index < 0) {
        var error = messageService.constructMessage('Invalid drink id!', 'Drink with id {0} is not found in the drink history.'.format(id));
        deferred.reject(error);
      } else {
    ...
    function deleteDrink(id) {
      var deferred = $q.defer(),
        ...
        index = drinks.indexOfObject('id', id);
      if (index < 0) {
        deferred.reject(messageService.constructMessage('Invalid drink id!', 'Drink with id {0} is not found in the drink history.'.format(id)));
      } else {
        drinks.splice(index, 1);
        setHistory(drinks);
        localStorageService.set('drink-state-changed', true);
        deferred.resolve(messageService.constructMessage('Deletion successful!', 'Drink with id {0} is removed from the drink history.'.format(id)));
      }
    ...
    function updateDrink(id, data) {
      ...
      if (isChanged) {
        var bodyWater = localStorageService.get('bodyWater', 0);
        if (bodyWater <= 0) {
          deferred.reject(messageService.constructMessage('Body water is not calculated!', 'Could not update information about drink: {0}.'.format(id)));
        } else {
          drink.BAC = calculationsService.calculateBAC(drink.quantity, drink.strength, bodyWater);
          localStorageService.set('drink-state-changed', true);
          setHistory(drinks);
